...
const URL=BASE+'/raluvaaa/solo-experience-v32/?qa=1&actor=A';
...
async function setup(type,viewport){
  ...
  await page.waitForFunction(()=>window.__RALUVAAA_V32__?.version===32&&window.__RV26_TEST__?.state&&window.__RALUVAAA_CREATE_AUDIO_V32__,{timeout:12000});
  return {browser,page};
}
...
async function createAndVerifySound(page){
  ...
  await page.waitForFunction(()=>window.__RALUVAAA_CREATE_AUDIO_V32__?.history?.some(x=>x.name==='create'),null,{timeout:5000});

  const info=await page.evaluate(async()=>{
    const a=window.__RALUVAAA_CREATE_AUDIO_V32__.audio;
    ...
    return {
      source:window.__RALUVAAA_CREATE_AUDIO_V32__.source,
      format:window.__RALUVAAA_CREATE_AUDIO_V32__.format,
      src:a.src,
  ...
  assert(info.src.includes('/raluvaaa/solo-experience-v32/create-a-wish.mp3'),'CREATE must use the V32 static MP3');
...
async function desktop(){
  ...
  try{
    assert.equal(await page.title(),'RALUVAAA · Experience V32');
    assert.equal(await page.evaluate(()=>window.RALUVAAA_STORE_KEY),'raluvaaaSoloExperienceV32R1');
    await assertLanguageSwitch(page,1600);
    await createAndVerifySound(page);
    console.log('RALUVAAA Experience V32 desktop language/audio QA passed');
...
async function mobile(){
  ...
  try{
    ...
    await createAndVerifySound(page);
    console.log('RALUVAAA Experience V32 WebKit/mobile language/audio QA passed');
  }finally{await browser.close()}
}
