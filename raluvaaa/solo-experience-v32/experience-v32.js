...
function decorateRail(){
  ...
  if(entrusted){
    ...
    const sig='v32-'+badge;
    if(entrusted.dataset.v32Rail!==sig){
      entrusted.dataset.v32Rail=sig;
      entrusted.innerHTML=railIcon(V32.entrusted)+badge;
    }
  }

  const my=document.getElementById('myWorldBtn');
  if(my&&my.dataset.v32Rail!=='1'){
    my.dataset.v32Rail='1';
    my.innerHTML=railIcon(V32.wishes);
  }
  if(my){
  ...
  if(create&&create.dataset.v32Rail!=='1'){
    create.dataset.v32Rail='1';
    create.innerHTML=railIcon(V32.create);
  }
  if(create){
  ...
  if(music&&music.dataset.v32Rail!=='1'){
    music.dataset.v32Rail='1';
    music.innerHTML=railIcon(V32.music,'<span class="sound-strike"></span>');
    try{
...
function decorateRoot(){
  document.querySelectorAll('#drawerBody .v28-root').forEach(root=>{
    if(root.querySelector('.v32-root-seed'))return;
    const old=root.querySelector('.v30-seed');
    if(!old)return;
    const holder=document.createElement('span');
    holder.className='v30-seed v32-root-seed';
    holder.innerHTML=V32.seed;
...
function buildDeck(){
  ...
  let deck=body.querySelector('.v32-action-deck');
  if(deck?.dataset.sig===sig)return;
  if(deck)deck.remove();

  deck=document.createElement('div');
  deck.className='v32-action-deck';
  deck.dataset.sig=sig;

  for(const entry of entries){
    const unit=document.createElement('div');
    unit.className='v32-action-unit';
    unit.style.color=entry.color;

    const button=document.createElement('button');
    button.type='button';
    button.className='v32-action-circle';
    ...
    label.className='v32-action-label';
...
function decorate(){
  document.title='RALUVAAA · Experience V32';
  decorateRail();
  decorateRoot();
  buildDeck();
}
