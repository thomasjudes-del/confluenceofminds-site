...
const POLICY=window.RALUVAAA_ENTRUSTED_POLICY_KEY||(window.RALUVAAA_SOLO?'raluvaaaEntrustedSoloV1':'raluvaaaEntrustedV26PolicyV2');
...
function installEngineGuard(){
  ...
  const syncMobileChrome=()=>{const minimap=doc.querySelector('.minimap');if(!minimap)return;minimap.style.setProperty('display',mobileViewportWidth()<=820?'none':'block','important')};
