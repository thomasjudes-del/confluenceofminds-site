...
const POLICY=window.RALUVAAA_SOLO?'raluvaaaEntrustedSoloV1':'raluvaaaEntrustedV26PolicyV2';
...
function installEngineGuard(){
  ...
  const mobileViewportWidth=()=>Math.min(...[window.innerWidth,window.visualViewport?.width,window.screen?.width].filter(Number.isFinite));
  const syncMobileChrome=()=>{const minimap=doc.querySelector('.minimap');if(!minimap)return;const mobile=mobileViewportWidth()<=820;minimap.style.setProperty('display',mobile?'none':'block','important');if(mobile){const c=minimap.querySelector('canvas');if(c){c.width=1;c.height=1}}};
  syncMobileChrome();window.addEventListener('resize',syncMobileChrome,{passive:true});window.visualViewport?.addEventListener('resize',syncMobileChrome,{passive:true});if(window.ResizeObserver)new ResizeObserver(syncMobileChrome).observe(document.documentElement);
