...
async function encourage(request,env,cors,wishId){
  const actorId=await actor(request,env,true),row=await wishById(env,wishId);if(!row||!row.is_public||row.state==='removed')fail(404,'wish_not_found','Wish not found');if(row.owner_actor_id===actorId)fail(409,'own_wish','Encourage another human\'s wish');try{await env.DB.prepare('INSERT INTO encouragements (wish_id,actor_id,created_at) VALUES (?,?,?)').bind(wishId,actorId,now()).run()}catch{fail(409,'already_encouraged','You already encouraged this wish')}return json({wishId,encouraged:true},201,cors);
}
