...
const BASE='http://127.0.0.1:4173/raluvaaa/solo-organic-v27/?soloqa=1&fresh=1';
...
(async()=>{
  ...
  assert.equal(await page.title(),'RALUVAAA · Solo Organic V27');
  ...
  assert.equal(await page.evaluate(()=>window.__RALUVAAA_V27__?.version),27,'V27 organic layer must be active');
  ...
  console.log('RALUVAAA Solo Organic V27 regression QA passed');
  await browser.close();
