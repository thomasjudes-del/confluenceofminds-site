...
const PATH='/raluvaaa/solo-experience-v41/';
...
async function setup(){
  ...
  await page.evaluate(()=>{
    localStorage.removeItem('raluvaaaSoloExperienceV41R1FreshStart');
    localStorage.setItem('raluvaaaSoloExperienceV40R1',JSON.stringify({
      version:26,lang:'en',entrusted:null,
      events:[{type:'create',actorId:'A',semanticId:'old-v40',lineageId:'old-v40-lineage',text:'OLD V40 WISH',loc:'Nantes, France'}]
    }));
    localStorage.setItem('raluvaaaSoloExperienceV41R1',JSON.stringify({
      version:26,lang:'en',entrusted:null,
      events:[{type:'create',actorId:'A',semanticId:'stale-v41',lineageId:'stale-v41-lineage',text:'STALE V41 WISH',loc:'Nantes, France'}]
    }));
  });
  ...
  await page.waitForFunction(()=>{
    ...
    return window.__RALUVAAA_V41__?.version===41 &&
      window.__RALUVAAA_WORKFLOW_V41__?.version===41 &&
      window.__RALUVAAA_REVIVE_AUDIO_V41__?.version===41 &&
...
async function assertFresh(page){
  const x=await page.evaluate(()=>({
    ...
    old:localStorage.getItem('raluvaaaSoloExperienceV40R1')
  }));
  assert.equal(x.reset?.performed,true,'V41 fresh reset must run');
  assert(x.reset.removed.includes('raluvaaaSoloExperienceV40R1'),'V41 must clear previous V40 local state');
  assert(x.reset.removed.includes('raluvaaaSoloExperienceV41R1'),'V41 must clear stale V41 state');
  assert.equal(x.state.events.length,0,'V41 must start without old human wishes');
  assert.equal(x.old,null,'old V40 local store must be gone');
}
...
async function testForeignActionSmoke(page,ownRoot){
  ...
  await page.evaluate(id=>{const m=window.__RV26_SOLO__.semantic().find(x=>x.semanticId===id);window.postMessage({type:'rv25-select',meta:m},location.origin)},foreign);
...
(async()=>{
  ...
  try{
    ...
    console.log('RALUVAAA V41 full workflow QC passed');
  }finally{
    await browser.close();
  }
