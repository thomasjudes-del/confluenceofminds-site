...
const MUSIC_URL='https://raw.githubusercontent.com/carolcarriazo/incarnation-game/main/public/music/Immersed.mp3';
const MUSIC_CREDIT='Immersed · Kevin MacLeod · CC BY 4.0';
const W=720,H=900,DURATION=9000,FPS=30,MUSIC_OFFSET=18;
...
function overlay(ctx,m,p,mode){
  ...
  ctx.fillStyle='rgba(236,245,244,.52)';ctx.font='500 9px system-ui,sans-serif';ctx.letterSpacing='.7px';
  ctx.fillText('confluenceofminds.com/raluvaaa',pad,874);
}
...
async function audioTrack(durationMs){
  ...
  try{
    ...
    const maxOffset=Math.max(0,buffer.duration-durationMs/1000-.25),offset=Math.min(MUSIC_OFFSET,maxOffset);
  ...
  }catch(e){console.warn('RALUVAAA share music unavailable',e);try{await ac.close()}catch{}return null}
}
...
function startPreviewAudio(){
  stopPreviewAudio();
  try{
    const a=new Audio(MUSIC_URL);a.crossOrigin='anonymous';a.preload='auto';a.volume=.28;
    const start=()=>{try{a.currentTime=Math.min(MUSIC_OFFSET,Math.max(0,(a.duration||MUSIC_OFFSET+12)-12))}catch{};const p=a.play();if(p?.catch)p.catch(()=>{})};
    a.addEventListener('loadedmetadata',start,{once:true});a.addEventListener('ended',()=>{try{a.currentTime=MUSIC_OFFSET;a.play().catch(()=>{})}catch{}});
    previewAudio=a;if(a.readyState>=1)start()
  }catch{}
}
...
function openShare(m){
  ...
  let host=document.getElementById('v44ShareOverlay');
  if(!host){host=document.createElement('div');host.id='v44ShareOverlay';document.body.appendChild(host)}
  host.innerHTML=modalHtml();host.classList.add('open');
  host.querySelector('.v44-share-x').onclick=closeShare;host.querySelector('.v44-share-scrim').onclick=closeShare;
...
function syncShareUi(){
  ...
  const name=host.querySelector('.v44-template-name');if(name)name.textContent=templateNames[shareStyle];
  const credit=host.querySelector('.v44-share-credit');if(credit)credit.classList.toggle('visible',shareMode==='animation');
  const status=host.querySelector('.v44-share-status');if(status)status.textContent=shareStatus;
  renderFrame(host.querySelector('canvas'),shareWish,shareStyle,shareMode==='image'?DURATION*.64:(performance.now()-shareStartedAt),'animation')
}
function animatePreview(){
  ...
  const step=now=>{
    ...
    else renderFrame(host.querySelector('canvas'),shareWish,shareStyle,DURATION*.64,'image');
...
function decorate(){
  document.title='RALUVAAA · Experience V44';
  const sub=document.querySelector('#brand .sub');if(sub)sub.textContent='EXPERIENCE V44';
