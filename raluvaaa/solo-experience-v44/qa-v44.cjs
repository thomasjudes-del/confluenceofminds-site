...
const PATH='/raluvaaa/solo-experience-v44/';
...
async function testCoreSmoke(page){
  const root=await createWish(page,'V44 core workflow root');
  ...
  await page.click('#confirm');
  await page.waitForFunction(()=>window.__RV26_SOLO__.semantic().filter(x=>x.kind==='split'&&x.lineageId===window.__RV26_SOLO__.semantic().find(y=>y.text==='V44 core workflow root')?.lineageId).length>=2,null,{timeout:7000});
  return root;
}
async function testSavedFollowsEvolution(page,root){
  ...
  let saved=await page.evaluate(()=>window.__RALUVAAA_V44__.saved());
  ...
  await page.fill('#evolveInput','V44 evolved current state');
  await page.click('#confirm');
  await page.waitForFunction(()=>window.__RV26_SOLO__.semantic().some(x=>x.text==='V44 evolved current state'),null,{timeout:7000});
  const evolved=await page.evaluate(()=>window.__RV26_SOLO__.semantic().find(x=>x.text==='V44 evolved current state').semanticId);

  const resolved=await page.evaluate(id=>window.__RALUVAAA_V44__.resolveSaved(id)?.semanticId,root);
...
async function testShare(page,id){
  ...
  await page.waitForSelector('#v44ShareOverlay.open',{timeout:5000});
  assert.equal(await page.locator('.v44-style-dots [data-style]').count(),5,'five share visual templates expected');
  assert.equal(await page.locator('.v44-share-modes [data-mode]').count(),2,'still and animated modes expected');
  assert.equal(await page.locator('[data-style-prev]').count(),1,'previous-style control required');
  assert.equal(await page.locator('[data-style-next]').count(),1,'next-style control required');

  const url=await page.evaluate(id=>window.__RALUVAAA_V44__.shareUrl(id),id);
  ...
  for(let style=0;style<5;style++){
    const value=await page.evaluate(async({id,style})=>{
      ...
      window.__RALUVAAA_V44__.renderFrame(c,id,style,3500,'image');
  ...
  const support=await page.evaluate(()=>window.__RALUVAAA_V44__.supportsAnimation());
  assert.equal(support,true,'Chromium QA should support animated clip export');
  const music=await page.evaluate(async()=>{
    const u=window.__RALUVAAA_V44__.musicUrl;
    const r=await fetch(u,{mode:'cors',cache:'no-store'});
    return{url:u,ok:r.ok,type:r.headers.get('content-type')||'',size:Number(r.headers.get('content-length')||0)};
  });
  assert(music.url.includes('Immersed.mp3'),'V44 must use a stable Immersed source');
  assert.equal(music.ok,true,'share music source must be fetchable from the browser');

  const clip=await page.evaluate(async id=>{
    const r=await window.__RALUVAAA_V44__.record(id,2,1100);
  ...
  assert.equal(clip.audioEmbedded,true,'animated share must contain an audio track');
  assert(/^video\//.test(clip.mime),'animated share must use a video MIME type');

  const canvas=page.locator('.v44-share-preview canvas');
  const box=await canvas.boundingBox();
  assert(box&&box.width>250&&box.height>300,'share preview must be usable on desktop');

  await page.click('.v44-share-x');
}
async function testMobile(page,id){
  ...
  await page.locator('#drawerBody .v33-action-unit[data-v39-icon="share"] .v33-action-circle').click();
  await page.waitForSelector('#v44ShareOverlay.open',{timeout:5000});
  const box=await page.locator('.v44-share-sheet').boundingBox();
  assert(box&&box.x>=0&&box.x+box.width<=390,'share sheet must stay inside mobile viewport');
  assert(box&&box.y>=0&&box.y+box.height<=844,'share sheet must fit in one mobile screen without page scroll');
  const footer=await page.locator('.v44-share-sheet footer').boundingBox();
  assert(footer&&footer.y+footer.height<=844,'mobile share actions must be visible without scrolling');
  const before=await page.evaluate(()=>window.__RALUVAAA_V44__.style());
  await page.locator('[data-style-next]').click();
  const after=await page.evaluate(()=>window.__RALUVAAA_V44__.style());
  assert.notEqual(after,before,'style navigation must change the share template');
  assert.equal(await page.locator('.v44-share-modes [data-mode]').count(),2);
  await page.click('.v44-share-x');
}
(async()=>{
  ...
  try{
    ...
    console.log('RALUVAAA V44 polished viral share + bookmarks QC passed');
