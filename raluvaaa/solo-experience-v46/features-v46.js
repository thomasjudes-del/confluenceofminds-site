...
function copy(){
  return lang()==='en'?{
    ...
    shareNow:'Share',download:'Download',copyLink:'Copy link',copiedButton:'✓ Link copied',shareLinkOnly:'Share link',clipReady:'Clip ready',close:'Close',
    ...
    shareNow:'Partager',download:'Télécharger',copyLink:'Copier le lien',copiedButton:'✓ Lien copié',shareLinkOnly:'Partager le lien',clipReady:'Clip prêt',close:'Fermer',
...
const SHARE_CLIP_DURATION=3000;
const SHARE_CLIP_FPS=24;
const SHARE_MUSIC_URL='./share-assets/immersed-share.m4a?build=v46-1';
const SHARE_MUSIC_OFFSET=0;
...
const V46_ASSET_BASE='../solo-experience-v45/share-assets/';
...
let prewarmTimer=0;
let mediaPrepareError=null;

function hash(s){let h=2166136261;for(const ch of String(s||'')){h^=ch.charCodeAt(0);h=Math.imul(h,16777619)}return h>>>0}
function assetUrl(kind,style){return V46_ASSET_BASE+(kind==='video'?'template-':'poster-')+(style+1)+(kind==='video'?'.mp4':'.png')+'?build=v46-final-20260925-2'}
...
function setStatus(msg){
  shareStatus=msg||'';
  const el=document.querySelector('#v46ShareOverlay .v46-share-status');
...
function setCopyButtonCopied(on){
  const btn=document.querySelector('#v46ShareOverlay [data-share-copy]');
  if(!btn)return;
...
async function loadVideoBlob(style){
  ...
  const p=fetch(assetUrl('video',style),{cache:'force-cache'})
    ...
    .catch(e=>{console.warn('V46 video preload failed',style,e);return null});
...
function loadPoster(style){
  ...
  }).catch(e=>{console.warn('V46 poster preload failed',style,e);return null});
...
function invalidatePrepared(){
  ...
  syncShareActionState()
}
async function shareMusicTrack(durationMs){
  const AC=window.AudioContext||window.webkitAudioContext;
  if(!AC)return null;
  const ac=new AC(),ctrl=new AbortController();
  const timer=setTimeout(()=>ctrl.abort(),800);
  try{
    const res=await fetch(SHARE_MUSIC_URL,{cache:'force-cache',signal:ctrl.signal});
    clearTimeout(timer);
    ...
    const maxOffset=Math.max(0,buffer.duration-durationMs/1000-.10);
    const offset=Math.min(SHARE_MUSIC_OFFSET,maxOffset);
    src.start(0,offset,durationMs/1000+.10);
    return{
    ...
    clearTimeout(timer);
    console.warn('V46 share music unavailable; continuing video-only',e);
...
async function recordPersonalizedClip(m,style){
  ...
  if(!mime)throw new Error('animation-unsupported');
  let canvas=document.querySelector('#v46ShareOverlay canvas.v46-pretty-animation');
  const isLive=!!canvas;
  if(!canvas){
...
function prewarmCurrentMedia(){
  ...
  prewarmTimer=setTimeout(()=>{
    ...
    prepareCurrentMedia().then(result=>{
      if(key!==mediaKey())return;
      if(!result?.file)throw new Error('media-not-ready');
      mediaPrepareError=null;
      setStatus('');
      syncShareActionState()
    }).catch(e=>{
      if(key!==mediaKey())return;
      mediaPrepareError=e||new Error('media-preparation-failed');
      console.warn('V46 media prewarm failed',e);
      setStatus('');
      syncShareActionState()
    })
  },0)
}
function syncShareActionState(){
  const btn=document.querySelector('#v46ShareOverlay [data-share-now]');
  if(!btn)return;
  const ready=!!(preparedMedia?.file&&preparedKey===mediaKey());
  const fallback=!!mediaPrepareError;
  btn.disabled=!ready&&!fallback;
  btn.classList.toggle('busy',!ready&&!fallback);
  btn.classList.toggle('fallback',fallback);
  btn.textContent=ready?copy().shareNow:(fallback?copy().shareLinkOnly:copy().preparingClip)
}
function setShareActionBusy(on){
  if(on){
    const btn=document.querySelector('#v46ShareOverlay [data-share-now]');
    if(btn){btn.disabled=true;btn.classList.add('busy');btn.textContent=copy().preparingClip}
  }else syncShareActionState()
}
function settleNativeShare(p,t){
  Promise.resolve(p).then(()=>{
    setStatus(t.shared);
    window.dispatchEvent(new CustomEvent('raluvaaa-share'))
  }).catch(e=>{
    if(e?.name==='AbortError')return;
    console.warn('V46 native share failed',e);
    copyUrl(shareUrl(shareWish)).then(()=>setStatus(t.fallback))
  })
}
function nativeShareNow(){
  if(!shareWish)return;
  const t=copy(),url=shareUrl(shareWish);
  const media=(preparedMedia?.file&&preparedKey===mediaKey())?preparedMedia:null;

  // Preparation failed: allow an immediate native link share rather than hanging.
  if(!media?.file){
    if(!mediaPrepareError)return;
    try{
      if(navigator.share){
        ...
        settleNativeShare(p,t);
        return
      }
    }catch(e){console.warn('V46 link share failed',e)}
  ...
  try{
    if(navigator.share&&navigator.canShare?.({files:[file]})){
      // Must be invoked synchronously inside the tap handler on iOS Safari.
      // The deep link appears once, inside text, because url + text duplicated it in WhatsApp.
      const p=navigator.share({
        title:'RALUVAAA · '+(shareWish.text||'Wish'),
  ...
  }catch(e){console.warn('V46 native share failed',e)}

  // Desktop browsers without Web Share: never open the video in a new tab.
  // Copy the wish deep link instead.
  copyUrl(url).then(()=>setStatus(t.fallback))
}
async function downloadCurrent(){
  if(!shareWish||recorderBusy)return;
  recorderBusy=true;
  const btn=document.querySelector('#v46ShareOverlay [data-share-download]');
  ...
  try{
    ...
    console.warn('V46 download failed',e);
...
function mediaHtml(){
  const cls=shareMode==='animation'?'v46-pretty-animation':'v46-pretty-still';
  return '<canvas class="v46-media '+cls+'" width="'+W+'" height="'+H+'"></canvas>'
}
function modalHtml(){
  const t=copy();
  return '<div class="v46-share-scrim"></div><section class="v46-share-sheet" role="dialog" aria-modal="true">'+
    '<header><div><span class="v46-eyebrow">RALUVAAA SHARE</span><h3>'+escapeHtml(t.shareTitle)+'</h3></div><button type="button" class="v46-share-x">×</button></header>'+
    '<div class="v46-share-modes"><button type="button" data-mode="image">'+escapeHtml(t.image)+'</button><button type="button" data-mode="animation">'+escapeHtml(t.animation)+'</button></div>'+
    '<div class="v46-share-preview">'+mediaHtml()+
      '<button type="button" class="v46-style-nav prev" data-style-prev>‹</button><button type="button" class="v46-style-nav next" data-style-next>›</button>'+
      '<div class="v46-style-dots">'+templateNames.map((n,i)=>'<button type="button" data-style="'+i+'" aria-label="'+n+'"></button>').join('')+'</div>'+
      '<div class="v46-swipe-hint">‹ '+(lang()==='en'?'swipe':'glisser')+' ›</div></div>'+
    '<div class="v46-share-meta"><span class="v46-audio-note">Immersed · Kevin MacLeod · CC BY 4.0</span><span class="v46-share-status"></span></div>'+
...
function syncPreview(){
  const host=document.getElementById('v46ShareOverlay');if(!host||!shareWish)return;
  ...
  const note=host.querySelector('.v46-audio-note');if(note)note.classList.toggle('visible',shareMode==='animation');
  const status=host.querySelector('.v46-share-status');if(status)status.textContent=shareStatus
}
function startPrettyAnimation(){
  cancelAnimationFrame(shareAnimationFrame);
  const host=document.getElementById('v46ShareOverlay');
  const canvas=host?.querySelector('canvas.v46-media');
  ...
  const step=now=>{
    const live=document.getElementById('v46ShareOverlay');
    const c=live?.querySelector('canvas.v46-pretty-animation');
...
function rebuildMedia(){
  const host=document.getElementById('v46ShareOverlay');if(!host)return;
  const preview=host.querySelector('.v46-share-preview'),old=preview.querySelector('.v46-media'),overlay=preview.querySelector('.v46-card-overlay');
...
function bindShareSwipe(host){
  const area=host.querySelector('.v46-share-preview');let sx=null,sy=null;
...
function openShare(m){
  ...
  let host=document.getElementById('v46ShareOverlay');
  if(!host){host=document.createElement('div');host.id='v46ShareOverlay';document.body.appendChild(host)}
  host.innerHTML=modalHtml();host.classList.add('open');
  host.querySelector('.v46-share-x').onclick=closeShare;host.querySelector('.v46-share-scrim').onclick=closeShare;
  ...
  bindShareSwipe(host);invalidatePrepared();setCopyButtonCopied(false);syncPreview();startPrettyAnimation();prewarmCurrentMedia();syncShareActionState()
}
function closeShare(){
  const host=document.getElementById('v46ShareOverlay');
  cancelAnimationFrame(shareAnimationFrame);
