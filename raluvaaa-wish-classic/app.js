...
function injectBridge(){
  ...
  script.textContent=`(function(){
    ...
    window.RV_BRIDGE={seedNodes,growWish,connect,touch,setEncouraged,fossil,focus,setWishNodes,nodeState:id=>{const n=node(id);return n?stateOf(n):null},worldClock:()=>world.clock};
...
function act(a,w){
  if(a==='encourage'){const i=state.encouraged.indexOf(w.id);if(i<0){state.encouraged.push(w.id);bridge.setEncouraged(w.nodeId,true);toast('You encouraged this wish.')}else{state.encouraged.splice(i,1);bridge.setEncouraged(w.nodeId,false);toast('Encouragement removed.')}save();openWish(w.id);return}
