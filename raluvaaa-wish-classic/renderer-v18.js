/* RALUVAAA V18 visual treatment over the exact historical v03 engine.
   Geometry stays historical. V18 restores visible organic pigment without returning to neon/glow. */
(function(){
  'use strict';
  ...
  function inject(){
    ...
    script.textContent=`(function(){
      try{
        /* Muted stellar / mineral palette. Hue varies again inside each lineage below. */
        const rootPalette=[205,228,312,27,158,274,190,16,140];
        ...
        /* Halos are atmospheric, not light sources. Enough pigment to keep the world alive at fit view. */
        rootHalo=function(r){
          ...
          let rad=(61+Math.sqrt(count)*8.8)*z;rad=clamp(rad,13,195);
          const gr=ctx.createRadialGradient(s.x,s.y,0,s.x,s.y,rad);
          gr.addColorStop(0,'hsla('+r.hue+',40%,72%,.050)');
          gr.addColorStop(.34,'hsla('+(r.hue+16)+',34%,62%,.021)');
          gr.addColorStop(.72,'hsla('+(r.hue-11)+',26%,52%,.006)');
          gr.addColorStop(1,'hsla('+r.hue+',20%,45%,0)');
          ctx.fillStyle=gr;ctx.beginPath();ctx.arc(s.x,s.y,rad,0,TAU);ctx.fill();
        };

        function encouragementLevel(obj){return obj&&obj.manualEncouraged?.64:0}

        function drawFlow(e,z,activity,state,h2){
          if(state==='fossil'||z<=.18)return;
          ...
          for(let i=0;i<dots;i++){
            ...
            const alpha=.14+a*.30;
            ctx.fillStyle='hsla('+h2+',34%,82%,'+alpha+')';
            ctx.shadowColor=a>.62?'hsla('+h2+',30%,78%,'+(alpha*.55)+')':'transparent';
            ctx.shadowBlur=a>.62?(1.5+a*3.5)/Math.max(.72,z):0;
        ...
        drawEdge=function(e,progress=1){
          ...
          const z=camera.zoom,state=stateOfEdge(e),fossil=state==='fossil',dormant=state==='dormant';
          const a=((hash('pig-a'+e.id)%1000)/1000-.5),b=((hash('pig-b'+e.id)%1000)/1000-.5),c=((hash('pig-c'+e.id)%1000)/1000-.5);
          const h=(e.hue+a*50+360)%360,h2=(e.hue+b*58+360)%360,h3=(e.hue+c*40+360)%360;
          const sat=35+(hash('pig-s'+e.id)%13),light=61+(hash('pig-l'+e.id)%10);
          ...
          if(alive){
            const alpha=e.mine?.74:clamp(.19+.19*Math.min(1,z*.9)+encouragement*.18,.19,.52);
            ctx.shadowColor=e.mine?'rgba(226,215,194,.30)':encouragement>.01?'rgba(224,239,229,'+(.13+encouragement*.50)+')':'hsla('+h+',34%,70%,.060)';
            ctx.shadowBlur=(e.mine?5:.7+encouragement*12)/Math.max(.48,z);
            const g=ctx.createLinearGradient(e.x0,e.y0,e.x1,e.y1);
            if(e.mine){
              g.addColorStop(0,'rgba(218,208,188,.68)');g.addColorStop(.48,'rgba(238,228,207,.74)');g.addColorStop(1,'rgba(201,198,184,.60)');
            }else{
              g.addColorStop(0,'hsla('+((h-24+360)%360)+','+Math.max(27,sat-5)+'%,'+Math.max(55,light-5)+'%,'+alpha+')');
              g.addColorStop(.25,'hsla('+((h2+11)%360)+','+sat+'%,'+Math.min(75,light+6)+'%,'+(alpha*.99)+')');
              g.addColorStop(.54,'hsla('+((h3-9+360)%360)+','+Math.max(25,sat-7)+'%,'+Math.min(73,light+3)+'%,'+(alpha*.94)+')');
              g.addColorStop(.79,'hsla('+((h+29)%360)+','+Math.max(24,sat-8)+'%,'+Math.max(56,light-2)+'%,'+(alpha*.88)+')');
              g.addColorStop(1,'hsla('+((h2+43)%360)+','+Math.max(22,sat-10)+'%,'+light+'%,'+(alpha*.80)+')');
            }
            ctx.strokeStyle=g;ctx.lineWidth=(e.mine?1.72:1.04+encouragement*.25)/Math.max(.48,z);pathEdge(e,progress,0,.34/Math.max(.62,z));ctx.stroke();ctx.shadowBlur=0;

            const pat=ctx.createLinearGradient(e.x0,e.y0,e.x1,e.y1);
            pat.addColorStop(0,'rgba(218,198,177,'+(alpha*.10)+')');
            pat.addColorStop(.32,'rgba(177,205,196,'+(alpha*.18)+')');
            pat.addColorStop(.68,'rgba(220,194,190,'+(alpha*.14)+')');
            pat.addColorStop(1,'rgba(184,195,215,'+(alpha*.09)+')');
            ...
            /* Dormant history keeps a faint pigment. Fossils are nearly mineral-grey, but not dead black. */
            ctx.shadowBlur=0;
            if(dormant){
              ctx.strokeStyle='hsla('+h2+',24%,63%,.145)';
              ctx.lineWidth=.68/Math.max(.52,z);pathEdge(e,1);ctx.stroke();
              ctx.strokeStyle='hsla('+h3+',18%,69%,.045)';
              ctx.lineWidth=.34/Math.max(.58,z);pathEdge(e,1,(hash('old-side'+e.id)%2?1:-1)*.58/Math.max(.74,z));ctx.stroke();
            }else{
              ctx.strokeStyle='hsla('+h+',12%,58%,.065)';
              ctx.lineWidth=.60/Math.max(.52,z);pathEdge(e,1);ctx.stroke();
            }
          }
        ...
        drawNode=function(n){
          ...
          const st=stateOf(n),tone=((hash('node-pig'+n.id)%1000)/1000-.5)*44,h=(n.root.hue+tone+360)%360;
          ...
          let r=(n.rootNode?3.05:(n.mine||n.received)?3.45:alive?1.34:.90)/Math.max(.55,z);
          r*=1+encouragement*.65;
          ctx.fillStyle=(n.mine||n.received)?'#ded5c5':encouragement>.01?'rgba(239,245,234,.92)':n.rootNode?'hsla('+h+',40%,79%,.67)':alive?'hsla('+h+',38%,75%,.50)':st==='dormant'?'hsla('+h+',22%,68%,.24)':'hsla('+h+',12%,61%,.11)';
          if(alive){ctx.shadowColor=(n.mine||n.received)?'rgba(223,210,186,.34)':encouragement>.01?'rgba(225,240,229,.82)':'hsla('+h+',34%,73%,.10)';ctx.shadowBlur=((n.mine||n.received)?5:encouragement>.01?encouragement*18:1.2)/Math.max(.62,z)}
