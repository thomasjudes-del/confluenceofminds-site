/* RALUVAAA V16 visual treatment over the exact historical v03 engine.
   Geometry stays historical; this file changes only rendering cues and controls. */
(function(){
  'use strict';
  ...
  function inject(){
    ...
    script.textContent=`(function(){
      try{
        ...
        function encouragementLevel(count){
          const c=Math.max(0,Number(count)||0);
          return c?Math.min(1,Math.log1p(c)/Math.log(101)):0;
        }
        ...
        drawEdge=function(e,progress=1){
          ...
          const encouragement=encouragementLevel(e.encouragementCount||0);
          const activity=Math.max(.04,Math.min(1,Number(e.activityLevel)||.12));
          const alive=state==='alive'||encouragement>0||activity>.18;
          ctx.lineCap='round';
          if(alive){
            const alpha=e.mine?.72:clamp(.13+.16*Math.min(1,z*.9)+encouragement*.18,.13,.48);
            ctx.shadowColor=e.mine?'rgba(226,215,194,.35)':encouragement>.01?'rgba(224,239,229,'+(.12+encouragement*.48)+')':'hsla('+h+',22%,70%,.035)';
            ctx.shadowBlur=(e.mine?5.5:.55+encouragement*11)/Math.max(.48,z);
            ...
            ctx.strokeStyle=g;ctx.lineWidth=(e.mine?1.72:1.0+encouragement*.20)/Math.max(.48,z);pathEdge(e,progress,0,.34/Math.max(.62,z));ctx.stroke();ctx.shadowBlur=0;
        ...
        drawNode=function(n){
          const z=camera.zoom;if(z<.15&&!n.rootNode&&!n.mine&&!(n.encouragementCount>0)&&!n.entrustedUrgency)return;if(!inView(n.x,n.y,115))return;
          const st=stateOf(n),tone=((hash('node-pig'+n.id)%1000)/1000-.5)*46,h=(n.root.hue+tone+360)%360;
          const encouragement=encouragementLevel(n.encouragementCount||0);
          ...
          r*=1+encouragement*.52;
          ctx.fillStyle=(n.mine||n.received)?'#ded5c5':encouragement>.01?'rgba(237,243,232,'+(.48+encouragement*.42)+')':n.rootNode?'hsla('+h+',24%,78%,.58)':alive?'hsla('+h+',22%,74%,.36)':st==='dormant'?'rgba(154,166,184,.18)':'rgba(112,124,142,.08)';
          if(alive){ctx.shadowColor=(n.mine||n.received)?'rgba(223,210,186,.38)':encouragement>.01?'rgba(225,240,229,'+(.18+encouragement*.60)+')':'transparent';ctx.shadowBlur=((n.mine||n.received)?5:encouragement*14)/Math.max(.62,z)}
          ...
          if(world.selected?.id===n.id){
            const phase=(tNow*.00012)%1;
            for(let i=0;i<3;i++){
              const q=(phase+i/3)%1,rr=(15+q*94)/Math.max(.60,z),aa=(1-q)*.40;
              ctx.strokeStyle='rgba(190,218,220,'+aa+')';ctx.lineWidth=.80/Math.max(.60,z);
              ctx.beginPath();ctx.arc(n.x,n.y,rr,0,TAU);ctx.stroke();
            }
            const breathe=.5+.5*Math.sin(tNow*.0018);
            ctx.strokeStyle='rgba(205,226,226,'+(.24+.08*breathe)+')';ctx.lineWidth=.72/Math.max(.62,z);
            ctx.beginPath();ctx.arc(n.x,n.y,(8.5+2*breathe)/Math.max(.60,z),0,TAU);ctx.stroke();
          }
        };
